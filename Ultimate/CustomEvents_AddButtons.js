 ...
 */
 (function (window) {
    ...
    link.setAttribute('href', 'https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&display=swap');
    ...
    let messageWrapperTimerId = setInterval(() => {
        const messageWrapper = document.querySelectorAll('.messageWrapper li.chatMessage.agent')
    ...
    (function setupCustomButtonSnapIn(liveAgentAPI) {
        ...
        function createCustomButtonElements(initialData, wrapper, rating, container) {
            ...
            if (rating) {
                ...
                initialData.forEach(({ text, link}) => {
                    ...
                    if(link && initialData.length === 1) {
                        buttonElement = createEl("button", {class: "button-link-wrapper"}, wrapper);

                        const linkElement = createEl("span", { class: "button-link"}, buttonElement);
                        linkElement.innerHTML = text;

                        createSvg(buttonElement, '0 0 16 16', '#5D5ADC', 'M3.33333 3.33333V12.6667H12.6667V8H14V12.6667C14 13.4 13.4 14 12.6667 14H3.33333C2.59333 14 2 13.4 2 12.6667V3.33333C2 2.6 2.59333 2 3.33333 2H8V3.33333H3.33333ZM9.33333 3.33333V2H14V6.66667H12.6667V4.27333L6.11333 10.8267L5.17333 9.88667L11.7267 3.33333H9.33333Z');

                        const noteBlock = createEl("div", {class: "button-note-block"}, container);

                        const noteText = createEl("span", {class: "button-note-text"}, noteBlock)
                        noteText.innerHTML = "Note: This page will open in a new browser tab"

                    } else {
                        buttonElement = createEl("button", {class: "btn-default"}, wrapper);
                        buttonElement.innerHTML = text;
                    }
    
                    buttonElement.addEventListener("click", () => customOnButtonClick(container, link, text));
                });
            }     
        }
        ...
        function addChatButtonsDefault (version, chatButtons, index, title) {
            ...
            var info = createEl("div", { class: `info info-v${version}` }, buttonContainer);  
            ...
            var buttonsWrapper = createEl("div", { class: `buttons-wrapper buttons-wrapper-v${version}`}, buttonContainer);   

            createCustomButtonElements(chatButtons, buttonsWrapper, rating, buttonContainer)
            addElementToMessageArea(buttonContainer, TYPE_BUTTONS, chatButtons, index, true);
        }
        ...
        function addChatButtonRating(initialData, wrapper, rating, container) {
            ...
            createEl("input", {class: "rating__input rating__input--none", name: "rating3", id: "rating3-none", value: "0", type: "radio", disabled: "disabled", checked: "checked"}, starsBlock);

            initialData.forEach(({text}, index) => {
                
                var specialId = Date.now();
                var starLabel = createEl("label", {class: "rating__label", ariaLabel: ` ${index + specialId}star`, for: `rating${index + specialId}`}, starsBlock);
                
                createSvg(starLabel, '0 0 22 22', undefined, 'M12.1017 1.69434C11.9064 1.26465 11.4767 0.991211 11.008 0.991211C10.5001 0.991211 10.0705 1.26465 9.87514 1.69434L7.37514 6.88965L1.75014 7.70996C1.28139 7.78809 0.890767 8.10059 0.734517 8.56934C0.617329 8.99902 0.734517 9.50684 1.04702 9.81934L5.10952 13.8428L4.17202 19.5459C4.09389 20.0146 4.2892 20.4834 4.67983 20.7568C5.07045 21.0693 5.57827 21.0693 5.96889 20.874L11.008 18.1787L16.008 20.874C16.4376 21.0693 16.9455 21.0693 17.3361 20.7568C17.7267 20.4834 17.922 20.0146 17.8439 19.5459L16.8673 13.8428L20.9298 9.81934C21.2814 9.50684 21.3986 8.99902 21.2423 8.56934C21.0861 8.10059 20.6955 7.78809 20.2267 7.70996L14.6408 6.88965L12.1017 1.69434Z', "#5D5ADC", "2");

                createEl("input", {class: "rating__input", name: "rating3", id: `rating${index + specialId}`, type: "radio", value: "1"}, starsBlock)

                starLabel.addEventListener("click", () =>  customOnButtonClick(container, "", text));
            });
           
            var buttonElement = createEl("button", {class: "btn-rating"}, wrapper);
            buttonElement.innerHTML = lastButtonElement.text

            buttonElement.addEventListener("click", () => customOnButtonClick(container, "", lastButtonElement.text));
        }

        var isMainTab = liveAgentAPI && liveAgentAPI.browserSessionInfo && liveAgentAPI.browserSessionInfo.isPrimary;
        var sessionId = liveAgentAPI && liveAgentAPI.connection && liveAgentAPI.connection.sid;
        ...
        function onCustomData(data, type, title) {
            ...
            try {
                const messagesList = document.querySelectorAll('.messageWrapper li.chatMessage.agent');
                const lastMessageItem = messagesList[messagesList.length - 1];
                if (lastMessageItem) {
                    lastMessageItem.querySelector('.chatContent')?.classList.add('custom-item');
                }
                ...
                if (type === TYPE_BUTTONS) {
                    ...
                    var buttonsRenderers = {
                        [IS_VERSION_1]: () => addChatButtonsDefault(1, eventData.chatButtons, undefined, titleMessage.replace(VERSION_1_KEY, '')),
                        [IS_VERSION_2]: () => addChatButtonsDefault(2, eventData.chatButtons, undefined, titleMessage.replace(VERSION_2_KEY, '')),
                        [IS_VERSION_3]: () => addChatButtonsDefault(3, eventData.chatButtons, undefined, titleMessage.replace(VERSION_3_KEY, '')),
                        default: () => addChatButtonsDefault(0, eventData.chatButtons, undefined, titleMessage),
                    }
                    ...
                    if(lastMessageItem){
                        ...
                        carouselMesasge = lastMessageItem.getElementsByTagName('span')[0]?.innerHTML
                        ...
                        if (eventData.cards.length >= 1) {
                            var carouselsRenderers = {
                                [IS_VERSION_1]: () => addChatCarousel(1, eventData, carouselMesasge),
                                [IS_VERSION_2]: () => addChatCarousel(2, eventData, carouselMesasge),
                                [IS_VERSION_3]: () => addChatCarousel(3, eventData, carouselMesasge),
                            ...
                            lastMessageItem.querySelector('.nameAndTimeContent').classList.add('hidden')
                            lastMessageItem.querySelector('.embeddedServiceLiveAgentStateChatAvatar').classList.add('hidden')
                    }
                }
            } catch (err) {
                console.log(err,'err')
        ...
        function onClickButtonElement(buttonLink, buttonText, cardIndex) {
            ...
            var serializedData = JSON.stringify({ text: buttonText, cardIndex: cardIndex });
            
            onButtonClickInAnyTab(serializedData);
        }

        function createSvg(container, viewBox, fill, path, stroke, strokeWidth) {
            var externalLinkSvg = createEl('svg', {
                viewBox
            }, container, true);
            ...
            }, externalLinkSvg, true);
        }

        function addButtonToContainer(container, button, carouselContainer, index) {
            var buttonText = button.text;
            var buttonLink = button.link;
            var buttonElement = createEl('button', {class: 'ultimate-card-button'}, container);
            
            if (buttonLink) {

                buttonElement.classList.add('ultimate-card-link-wrapper');

                const buttonTextEl = createEl('span', {class: 'button-link'}, buttonElement);
                buttonTextEl.innerHTML = buttonText;

                createSvg(buttonElement, "0 0 16 16", "#FFFFFF", 'M3.33333 3.33333V12.6667H12.6667V8H14V12.6667C14 13.4 13.4 14 12.6667 14H3.33333C2.59333 14 2 13.4 2 12.6667V3.33333C2 2.6 2.59333 2 3.33333 2H8V3.33333H3.33333ZM9.33333 3.33333V2H14V6.66667H12.6667V4.27333L6.11333 10.8267L5.17333 9.88667L11.7267 3.33333H9.33333Z');
        ...
        function addElementToMessageArea(element, type, data, index, isVersionElement) {
            ...
            if (messageArea) {
                ...
                if (messageList) {
                    ...
                    var elementIndex = isIndexLast ? messageList.children.length : parseInt(index);
                    ...
                    if (data && isIndexLast) {
                        ...
                        localStorage.setItem(CUSTOM_ELEMENTS_KEY, JSON.stringify(CUSTOM_ELEMENTS_DATA));
                    }

                    if (messageList.children.length === elementIndex) {
                       
                        if(isVersionElement) {
                            var lastMessageItem = messageList.children[messageList.children.length - 1];
                            if (lastMessageItem) {
                                var contentWrapper = lastMessageItem.querySelector('.chatContent');
                                ...
                                lastMessageItem.querySelector('.loader')?.classList.add('hidden');
                            }
                        ...
                        var observer = new MutationObserver(function (mutationsList, observer) {
                            ...
                            if (getElementIndex(element) !== elementIndex) {
                                observer.disconnect();
                                var markerElement = element.parentNode && element.parentNode.children[elementIndex - 1];
                                if (markerElement) {
                        ...
                        messageList.children[elementIndex - 1].insertAdjacentElement('afterend', element);
                    }
                    ...
                    if (type === TYPE_CAROUSEL) {
                        buttonContainer = buttonContainer.querySelectorAll('.ultimate-card')[elementData.activeCard];
                    ...
                    if (parseInt(elementData.activeButton) >= 0) {
                        buttonContainer.querySelectorAll('button')[elementData.activeButton].classList.add('ultimate-btn-active');
                    }
        ...
        function createEl(tag, attrs, parent, isSvg) {
            ...
            var el = isSvg ? document.createElementNS(xmlns, tag) : document.createElement(tag);
        ...
        function dragMove(event) {
            ...
            if (shouldMoveSlider) {
                ...
                var xDelta = x1 - x2;
                if ((x1 < x2 && currentIndex === 0) || (x1 > x2 && currentIndex === itemsLength - 1)) {
                    xDelta = xDelta / 3;
                }
                carouselTrack.style = 'transform: translate3d(' + (slideWidth * -currentIndex - xDelta) + 'px, 0, 0); transition: none;';
            } else {
        ...
        function dragEnd(event) {
            ...
            if (busy && x1 && x2) {
                ...
                if (Math.abs(x1 - x2) > slideWidth / 3) {
                    if (x1 < x2) {
                        if (currentIndex - 1 >= 0) {
                            prevSlider({ target: carousel.querySelector('.ultimate-carousel-list') });
                            keepCurrentIndex = false;
                        }
                    } else {
                        if (currentIndex + 1 < itemsLength) {
                            nextSlider({ target: carousel.querySelector('.ultimate-carousel-list') });
                            keepCurrentIndex = false;
                        }
                    }
                }
        ...
        function moveSlider(buttonEl, isPrev, noAnimate, targetIndex) {
            ...
            if (!busy && goToIndex >= 0 && goToIndex < itemsLength) {
                ...
                carouselTrack.style = 'transform: translate3d(' + (slideWidth * -goToIndex) + 'px, 0, 0);' + (noAnimate ? ' transition: none;' : '');
                ...
                if (parseInt(CUSTOM_ELEMENTS_DATA[elementIndex].currentCard) !== goToIndex) {
                    CUSTOM_ELEMENTS_DATA[elementIndex].currentCard = goToIndex;
                    localStorage.setItem(CUSTOM_ELEMENTS_KEY, JSON.stringify(CUSTOM_ELEMENTS_DATA));
                    broadcastStorageEvent(CUSTOM_ELEMENTS_EVENT, JSON.stringify({ index: elementIndex }));
                }
            }
        }
        ...
        function customOnButtonClick(container, link, text) {
            const isDisabled =
              container.querySelector(".button-disabled") || container.querySelector(".star-disabled");
            if (isDisabled) {
              return;
            } else {
        ...
        function addChatCarousel(version, data, message, index) {
            try {
                ...
                var carouselContainer = createEl('div', {
                    class: 'ultimate-carousel-container ultimate-custom-carousel-container ultimate-hidden'
                });

                var carouselElement = createEl('div', {
                ...
                var buttonPrev = createEl('button', {
                    type: 'button',
                ...
                if (hasCarouselCards) {
                    for (let i = 0; i < parsedData.length; i++) {
                        ...
                        var infoContainer = createEl('div', {
                            class: `ultimate-card-info-block ultimate-card-info-block-v${version}`
                        }, cardElement);

                        if (parsedData[i].title) {
                            var cardTitle = createEl('h4', {class: "ultimate-card-title"}, infoContainer);
                            cardTitle.innerHTML = parsedData[i].title;
                        }
                        if (parsedData[i].description) {
                            var cardDescription = createEl('div', {class: "ultimate-custom-card-container"}, infoContainer);
                ...
                var buttonNext = createEl('button', {
                    type: 'button',
                    class: 'ultimate-carousel-button ultimate-carousel-next'
                }, carouselElement);
                buttonNext.innerText = 'Next';

                createSvg(buttonNext, '0 0 14 12', '#15142D', 'M13.5468 6.49778L9.12014 10.9422C8.97792 11.0844 8.80013 11.1556 8.62235 11.1556C8.44457 11.1556 8.2668 11.0844 8.12458 10.9422C7.84013 10.6578 7.84013 10.2133 8.12458 9.92889L11.3424 6.69334H0.960135C0.569023 6.69334 0.249023 6.37334 0.249023 5.98223C0.249023 5.59112 0.569023 5.27112 0.960135 5.27112H11.3424L8.12458 2.03557C7.84013 1.75112 7.84013 1.30668 8.12458 1.02223C8.40902 0.737788 8.85347 0.737788 9.13791 1.02223L13.5646 5.46668C13.8312 5.7689 13.8312 6.23112 13.5468 6.49778Z');

                buttonNext.addEventListener('click', nextSlider);

                addElementToMessageArea(carouselContainer, TYPE_CAROUSEL, parsedData, index, true);
            } catch (err) {
        ...
        function removeCustomElements() {
            ...
            for (var i = 0; i < containers.length; ++i) {
                if(!containers[i].classList.contains('version-custom-button') && !containers[i].classList.contains('ultimate-custom-card-buttons')) {
        ...
        function updateCustomElements(data) {
            CUSTOM_ELEMENTS_DATA = JSON.parse(localStorage.getItem(CUSTOM_ELEMENTS_KEY)) || {};
            ...
            if (elementData.type === TYPE_CAROUSEL) {
                ...
                carousel.dataset.currentIndex = elementData.currentCard;
                updateSlider(carousel, true);
                buttonContainer = element.querySelectorAll('.ultimate-card')[elementData.activeCard];
            ...
            if (elementData.activeButton >= 0) {
                ...
                var currentButton = buttonContainer.querySelectorAll('button')[elementData.activeButton]
                if (currentButton) {
        ...
        function registerAllMessages() {
            const payloadMessages = Array.from(document.getElementsByClassName(
              "chatMessage"
            ));
        ...
        function checkPendingAgentMessages() {
            ...
            var messages = Array.from(document.getElementsByClassName(
              "chatMessage"
            ));
        ...
        function onCustomEventInMain(result, elementType) {
            ...
            const eventKey = elementType === TYPE_CAROUSEL ?  ADD_CAROUSEL_EVENT : ADD_BUTTONS_EVENT;
        ...
        function attachChasitorCustomEventListener() {
            try {
                liveAgentAPI.addCustomEventListener(TYPE_BUTTONS, result => onCustomEventInMain(result, TYPE_BUTTONS));
                liveAgentAPI.addCustomEventListener(TYPE_CAROUSEL, result => onCustomEventInMain(result, TYPE_CAROUSEL));
            } catch (err) {
        ...
        function checkMessageArea() {
            var sidebarBody = document.querySelector('.embeddedServiceSidebar .sidebarBody');
            var observer = new MutationObserver(function (mutationsList, observer) {
        ...
        function onMessageSendAgent() {
            setTimeout(() => {
                ...
                const agentMessageCountStorage = localStorage.getItem('agentMessagesCount') || 0;
        ...
        window.embedded_svc.addEventHandler('onChasitorMessage', onMessageReceivedUser);
        window.embedded_svc.addEventHandler('onAgentMessage', onMessageSendAgent);
        window.embedded_svc.addEventHandler('onChatRequestSuccess', onChatRequestSuccess);
