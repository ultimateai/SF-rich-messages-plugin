  ...
  (function setupCustomButtonSnapIn(liveAgentAPI) {
      ...
      function createCustomButtonElements(initialData, wrapper, rating, container) {
          ...
          if (rating) {
              ...
              var buttonElement = createEl("button", {class: "btn-version3"}, wrapper);
              ...
              initialData.forEach(({ text, link}) => {
                  var buttonElement = createEl("button", {class: "btn-version1"}, wrapper);
                  
                  if(link && initialData.length === 1) {
                      buttonElement.classList.add("btn-version2")

                      const linkElement = createEl("span", { class: "button-v2-link"}, buttonElement);
                      linkElement.innerHTML = text;

                      const noteBlock = createEl("div", {class: "button-v2-note-block"}, container);

                      const noteText = createEl("span", {class: "button-v2-note-text"}, noteBlock)
      ...
      function addChatButtonsDefault (chatButtons, index, title) {
          ...
          var buttonsWrapper = createEl("div", { class: "buttons-wrapper buttons-wrapper-default"}, buttonContainer);   

          createCustomButtonElements(chatButtons, buttonsWrapper, undefined, buttonContainer)
      ...
      function addChatButtonsVersion1 (chatButtons, index, title) {
          ...
          var buttonsWrapper = createEl('div', {class: "buttons-wrapper buttons-wrapper-v1"}, buttonContainer);          
      ...
      function addChatButtonsVersion3 (chatButtons, index, title) {
          ...
          var buttonsWrapper = createEl("div", {class: "buttons-wrapper buttons-wrapper-v3"}, buttonContainer);
          ...
          try {
              ...
              if (type === TYPE_BUTTONS) {
                  ...
                  var buttonsRenderers = {
                      [IS_VERSION_1]: () => addChatButtonsVersion1(eventData.chatButtons, undefined, titleMessage.replace(VERSION_1_KEY, '')),
                      [IS_VERSION_3]: () => addChatButtonsVersion3(eventData.chatButtons, undefined, titleMessage.replace(VERSION_3_KEY, '')),
                      default: () => addChatButtonsDefault(eventData.chatButtons, undefined, titleMessage),
                  }
                  ...
                  if(lastMessageItem){
                      ...
                      if (eventData.cards.length >= 1) {
                          var carouselsRenderers = {
                              [IS_VERSION_1]: () => addChatCustomCarousel(1, eventData, carouselMesasge),
                              [IS_VERSION_2]: () => addChatCustomCarousel(2, eventData, carouselMesasge),
                              [IS_VERSION_3]: () => addChatCustomCarousel(3, eventData, carouselMesasge),
      ...
      function addButtonToContainer(container, button, cardIndex) {
          ...
          if (buttonLink) {
              ...
              var buttonLinkSvg = createEl('svg', {
                  viewBox: '0 0 19 18'
              }, buttonElement, true);
              createEl('path', {
                  fill: '#757575',
          ...
          buttonElement.addEventListener('click', function () {
              ...
              if (CUSTOM_ELEMENTS_DATA[elementIndex]) {
                  ...
                  CUSTOM_ELEMENTS_DATA[elementIndex].activeButton = getElementIndex(buttonElement);
                  ...
                  if (CUSTOM_ELEMENTS_DATA[elementIndex].type === TYPE_BUTTONS && !buttonLink) {
                      buttonElement.closest('.ultimate-btn-container').classList.add('ultimate-hidden');
                      setTimeout(function () {
      ...
      function addChatCarouselDefault(carousel, index) {
          try {
              ...
              var buttonPrevSvg = createEl('svg', {
                  viewBox: '0 0 8 13'
              }, buttonPrev, true);
              createEl('path', {
                  fill: '#172d3d',
              ...
              var carouselTrack = createEl('div', { class: 'ultimate-carousel-track' }, carouselList);
              if (hasCarouselCards) {
              ...
              var buttonNextSvg = createEl('svg', {
                  viewBox: '0 0 8 13'
              }, buttonNext, true);
              createEl('path', {
                  fill: '#172d3d',
              ...
              addElementToMessageArea(carouselContainer, TYPE_CAROUSEL, carousel, index);
              // Slider events
      ...
      function addChatCustomCarousel(version, data, message, index) {
          try {
              ...
              var info = createEl('div', {class: "ultimate-carousel-title"}, carouselElement)
              ...
              if (hasCarouselCards) {
                  for (let i = 0; i < parsedData.length; i++) {
                      ...
                      if (parsedData[i].title) {
                          var cardTitle = createEl('h4', {class: "ultimate-card-title"}, cardElement);
                          cardTitle.innerHTML = parsedData[i].title;
                      }
                      if (parsedData[i].description) {
                          var cardDescription = createEl('div', {class: "ultimate-custom-card-container"}, cardElement);
                      ...
                      if (parsedData[i].buttons && Array.isArray(parsedData[i].buttons)) {
                          var cardButtons = createEl('div', {
                              class: 'version-custom-button ultimate-custom-card-buttons ultimate-btn-container'
                          }, cardElement);
  
                          var buttonElement = createEl("button", {class: "btn-version1"}, cardButtons);
                  ...
                  carouselList.addEventListener('mousedown', dragStart);
                  carouselList.addEventListener('mousemove', dragMove);
                  carouselList.addEventListener('mouseup', (event) => dragEnd(event, dotsContainer));
                  carouselList.addEventListener('mouseleave', (event) => dragEnd(event, dotsContainer));
                  carouselList.addEventListener('touchstart', dragStart);
                  carouselList.addEventListener('touchmove', dragMove);
                  carouselList.addEventListener('touchend', (event) => dragEnd(event, dotsContainer));
              }
